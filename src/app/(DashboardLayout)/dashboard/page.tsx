    ...
    <div className="relative w-full min-h-screen pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <motion.section className="mb-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            { label: 'Profile Views', value: stats.views, icon: <BarChart3 className="h-5 w-5 text-purple-300" />, color: 'from-purple-500/20 to-pink-500/20' },
            { label: 'Active Listings', value: stats.listings, icon: <ShoppingBag className="h-5 w-5 text-blue-300" />, color: 'from-blue-500/20 to-cyan-500/20' },
            { label: 'Messages', value: stats.messages, icon: <Gift className="h-5 w-5 text-amber-300" />, color: 'from-amber-500/20 to-orange-500/20' },
            { label: 'Favorites', value: stats.favorites, icon: <Star className="h-5 w-5 text-pink-300" />, color: 'from-pink-500/20 to-rose-500/20' },
            { label: 'Completed Sales', value: stats.sales, icon: <Tag className="h-5 w-5 text-emerald-300" />, color: 'from-emerald-500/20 to-teal-500/20' }
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
              custom={i}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <Link href={action.path} className="block">
                      <div className="flex items-center gap-4 bg-white/5 hover:bg-white/10 transition-colors duration-200 p-4 rounded-lg border border-white/10 group">
                        ...
                        <div className="flex-1">
                          <h3 className="font-medium text-white group-hover:text-purple-200 transition-colors">{action.title}</h3>
          ...
          >
            <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-6 shadow-xl h-full">
                    ...
                    <div className="relative">
                      ...
                      <div className="flex gap-4 group">
                        ...
                        <div className="flex-1 bg-white/5 hover:bg-white/10 transition-colors duration-200 p-4 rounded-lg border border-white/5">
                          <p className="text-white group-hover:text-purple-200 transition-colors">{activity.title}</p>
              ...
              >
                <Link href="/all-lists">
                  <Button variant="outline" className="bg-white/10 hover:bg-white/20 border-white/10 text-white">
                    Explore Marketplace
                    <motion.div
                      animate={{ x: [0, 5, 0] }}
          ...
          <div className="backdrop-blur-md bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-white/10 rounded-xl p-6 shadow-xl overflow-hidden relative">
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-center mb-4">
                ...
                >
                  <Button size="sm" variant="ghost" className="text-white/70 hover:text-white rounded-full size-8 p-0">
                    <ChevronRight className="h-5 w-5" />
                  </Button>
                </motion.div>
              </div>
              
              <p className="text-white/70">Your listings are performing 15% better than last month. Keep it up!</p>
                ...
                <div className="bg-white/5 backdrop-blur-sm p-4 rounded-lg border border-white/10">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-white font-medium mt-1">Vintage Camera</p>
                    </div>
                ...
                <div className="bg-white/5 backdrop-blur-sm p-4 rounded-lg border border-white/10">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-white font-medium mt-1">Leather Jacket</p>
                    </div>
                    <div className="text-emerald-400 text-xs flex items-center">
