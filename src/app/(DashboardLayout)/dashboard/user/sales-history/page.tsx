...
import { SalesHistoryResponse } from "@/types/sales/sales";

export default function SellsHistoryPage() {
  const [data, setData] = useState<SalesHistoryResponse[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const sellsListings = await getSellsHistory(userId)

        console.log('sellsListings', sellsListings)

        setData(sellsListings?.data?.result || [])
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-950 to-violet-900 p-4 md:p-8">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          >
            <h3 className="text-white/70 text-sm font-medium mb-1">Total sales items</h3>
