...
export function DataTable<TData, TValue>({
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-8 px-4">
      ...
      <h3 className="text-lg font-semibold text-white mb-1">{error ? 'Something went wrong' : 'No listings found'}</h3>
      <p className="text-sm text-white/70 text-center mb-3">
        {error || "You don't have any listings yet. Create your first listing to start selling!"}
      </p>
      <div className="flex items-center justify-center">
        {!error && (
          <a href="/dashboard/user/listings/create-listing" className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg text-white font-medium hover:from-purple-700 hover:to-pink-700 transition-all duration-300 shadow-lg shadow-purple-900/30">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-60 text-center bg-transparent">
                  <EmptyState />
                </TableCell>
              </TableRow>
            )}
